...
- **h=1:** лучший MAE/WAPE — **D + SAN (28.95 / 11.10)**, лучший SMAPE — **D + TimesNet (18.57)**; все три D-конфига опережают одиночные идеи (B, C) и базлайны (TFT/N-BEATS/TCN).
...
### Как пополнять
Добавлять новую строку метода в обе таблицы, беря числа из ячейки **«1b. Метрики по зрелым фолдам»**
соответствующего ноутбука (последние 2 фолда). Учтены: A, B, C, D, D + SAN, D + TimesNet. Дальше — по мере новых прогонов (бленды/ансамбли, иерархия+MinT).
