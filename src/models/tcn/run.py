...
from .pipeline import TCNTrainConfig, run_tcn_backtest, run_tcn_fast_experiment


def parse_args():
    parser = argparse.ArgumentParser(description="TCN passenger-flow experiment.")
    ...
    parser.add_argument("--synth-days", type=int, default=60)
    parser.add_argument("--train-hours", type=int, default=96, help="Fast protocol real train length.")
    parser.add_argument("--past-window", type=int, default=48)
...
def main():
    ...
    if args.protocol == "fast":
        results, summary, history, synth_validation = run_tcn_fast_experiment(
            pivot,
        ...
        for horizon in sorted(results["horizon"].unique()):
            results_path = config.OUTPUT_DIR / f"tcn_results_{horizon}h.csv"
            summary_path = config.OUTPUT_DIR / f"tcn_summary_{horizon}h.csv"
        ...
        history_path = config.OUTPUT_DIR / "tcn_history_fast.csv"
        validation_path = config.OUTPUT_DIR / "tcn_synth_validation_fast.csv"
        ...
        for horizon in args.horizons:
            ...
            results, summary, history, synth_validation = run_tcn_backtest(
                pivot,
                horizon=horizon,
                train_modes=args.train_modes,
                synth_days=args.synth_days,
            ...
            results_path = config.OUTPUT_DIR / f"tcn_results_{horizon}h.csv"
            summary_path = config.OUTPUT_DIR / f"tcn_summary_{horizon}h.csv"
            history_path = config.OUTPUT_DIR / f"tcn_history_{horizon}h.csv"
            validation_path = config.OUTPUT_DIR / f"tcn_synth_validation_{horizon}h.csv"
    ...
    save_generation_config(config.OUTPUT_DIR / "tcn_generation_config.json", args.seed, args.synth_days, all_validation)
