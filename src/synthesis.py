...
def prepend_synthetic_week(real_pivot, seed=0):
    """Генерирует синтетическую неделю с индексом ДО реальных данных."""
    synth = synthesize_from_train(real_pivot, gen_days=7, seed=seed)
    real_start = real_pivot.index[0]
    synth_end = real_start - pd.Timedelta(hours=1)
    synth_start = synth_end - pd.Timedelta(hours=len(synth) - 1)
    synth.index = pd.date_range(start=synth_start, periods=len(synth), freq="h")
    synth = synth[real_pivot.columns]
    return synth
